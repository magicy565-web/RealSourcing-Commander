/**
 * Phase 5 — Sprint 5.1
 * Facebook 社媒管理模块 — 高保真原型
 *
 * 定位：Commander 是 OpenClaw 操作员的指挥台。
 * OpenClaw 在云电脑上直接操作浏览器抓取 Facebook 评论/私信，
...
function IntentBadge({ intent, score }: { intent: MsgIntent; score: number }) {
  const m: Record<MsgIntent, { l: string; c: string; bg: string; i: string }> = {
    inquiry:   { l: "询价", c: "#1877f2", bg: "#1877f218", i: "💰" },
...
export default function FacebookManager() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [msgRes, statsRes, accRes, tplRes] = await Promise.all([
        socialApi.getMessages({ platform: "facebook", limit: "50" }),
        socialApi.getStats(),
        socialApi.getAccounts(),
        socialApi.getTemplates({ platform: "facebook" }),
      ]);
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      toast.success("指令已下发给 OpenClaw", {
        description: `OpenClaw 将以 ${fbAccount?.accountName ?? "账号"} 身份在 Facebook 上发送回复`,
      });
  ...
  const opsUsedPct = fbAccount ? (fbAccount.dailyOpsUsed / fbAccount.dailyOpsLimit) * 100 : 0;
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "oklch(0.13 0.02 250)", color: "white" }}>
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-3 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-bold truncate">{fbAccount?.accountName ?? "Facebook 管理"}</p>
          </div>
          <p className="text-xs text-slate-500">OpenClaw 指挥台 · Phase 5</p>
        </div>
        {fbAccount && <HealthDot status={fbAccount.healthStatus} />}
        ...
        <div className="mx-4 mb-3 px-3.5 py-2.5 rounded-2xl flex-shrink-0"
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-mono font-bold" style={{ color: opsColor }}>
              {fbAccount.dailyOpsUsed} / {fbAccount.dailyOpsLimit}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 mt-1.5">
            ...
            {highValueCount > 0 && (
              <span className="text-xs font-semibold text-blue-400">⭐ {highValueCount} 高价值线索</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-1 mx-4 mb-3 p-1 rounded-2xl flex-shrink-0"
        ...
        {[
          { key: "messages",  label: "消息处理", icon: MessageCircle },
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
            ...
            style={{
              background: activeTab === tab.key ? "#1877f2" : "transparent",
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        {loading ? (
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
              {[
                ...
                <button key={f.key} onClick={() => setMsgFilter(f.key as any)}
                  ...
                  style={{
                    background: msgFilter === f.key ? "#1877f2" : "oklch(0.19 0.02 250)",
                    color: msgFilter === f.key ? "white" : "#64748b",
                    border: `1px solid ${msgFilter === f.key ? "#1877f2" : "oklch(1 0 0 / 8%)"}`,
                  }}>
                  {f.label}
                </button>
              ))}
                ...
                {filteredMessages.map(msg => (
                  <button key={msg.id} onClick={() => openMsg(msg)}
                    ...
                    style={{
                      ...
                      border: `1px solid ${msg.isHighValue ? "#1877f230" : "oklch(1 0 0 / 7%)"}`,
                    }}>
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
              {["all", ...templateCategories].map(cat => (
                <button key={cat} onClick={() => setTemplateFilter(cat)}
                  className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all"
                  style={{
                    background: templateFilter === cat ? "#1877f2" : "oklch(0.19 0.02 250)",
                    color: templateFilter === cat ? "white" : "#64748b",
                    border: `1px solid ${templateFilter === cat ? "#1877f2" : "oklch(1 0 0 / 8%)"}`,
                  }}>
                  {cat === "all" ? "全部" : cat}
                </button>
              ))}
          ...
          <div className="space-y-3">
            {stats?.overall && (
              <div className="rounded-2xl p-4"
                ...
                <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                  <BarChart2 className="w-4 h-4 text-blue-400" />Facebook 消息统计
                </h3>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    { label: "询价意图", value: stats.overall.inquiries, color: "#1877f2" },
            ...
            <div className="rounded-2xl p-4"
              style={{ background: "oklch(0.17 0.02 250)", border: "1px solid #1877f220" }}>
              <h3 className="text-sm font-bold text-blue-400 mb-3 flex items-center gap-2">
                <Eye className="w-4 h-4" />OpenClaw 工作流说明
              </h3>
              <div className="space-y-2.5">
                {[
                  "OpenClaw 在云电脑上登录 Facebook，自动抓取帖子评论和私信",
                  ...
                  <div key={i} className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                      style={{ background: "#1877f2", color: "white" }}>{i + 1}</span>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            ...
            <div className="rounded-2xl p-3.5"
              ...
              <p className="text-xs text-slate-500 mb-2">
                {selectedMsg.message_type === "comment" ? "💬 Facebook 评论" : "✉️ Messenger 私信"}
              ...
              <>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setShowTemplates(!showTemplates)}
                    ...
                    style={{
                      background: showTemplates ? "#1877f220" : "oklch(0.22 0.02 250)",
                      color: showTemplates ? "#1877f2" : "#94a3b8",
                      border: `1px solid ${showTemplates ? "#1877f240" : "oklch(1 0 0 / 8%)"}`,
                    }}>
                ...
                {showTemplates && (
                  <div className="rounded-2xl overflow-hidden"
                    style={{ border: "1px solid #1877f230" }}>
                    ...
                    <div className="max-h-48 overflow-y-auto"
                      style={{ background: "oklch(0.17 0.02 250)" }}>
                      {templates
                        .filter(t => t.platform === "facebook" || t.platform === "all")
                        .map(tpl => (
